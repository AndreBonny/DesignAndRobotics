...
void draw_face_boxes(dl_matrix3du_t *image_matrix, box_array_t *boxes) {
  ...
  for (int i = 0; i < boxes->len; i++) {
    ...
    /*if(img_center_x < (face_center_x - confidence)) {
      if(pan_position + confidence >= 0)
      ...
      if(img_center_x > (face_center_x + confidence)) {
      if(pan_position + confidence <= 180)
        pan_position += movement;
      else
        pan_position = 180;
      ledcAnalogWrite(pan_ch, pan_position);
      ...
      if(img_center_y < (face_center_y - confidence)) {
      if(tilt_position - confidence >= 100)
        tilt_position -= movement;
      else
        tilt_position = 100;
      ledcAnalogWrite(tilt_ch, tilt_position);
      delay(100);
      ...
      if(img_center_y > (face_center_y + confidence)) {
      if(tilt_position + confidence <= 180)
        tilt_position += movement;
      else
        tilt_position = 180;
      ledcAnalogWrite(tilt_ch, tilt_position);
    ...
    // E][esp_jpg_decode.c:118] esp_jpg_decode(): JPG Decompression Failed! Device error or wrong termination of input stream
...
void Face_tracking() {
  ...
  t = millis() - t;
  Serial.printf("TEMPO = %lu", t);
  if (boxes != NULL) {
    Serial.printf("################### Face detected!\n");
